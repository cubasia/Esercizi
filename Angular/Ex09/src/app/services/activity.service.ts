...
export class ActivityService {
  ...
  getWithParameters(parameters: string[]): Observable<Attivita>  {
    ...
    // if (activityParams?.price)
    //   params = params.set('price', activityParams.price);
    // if (activityParams?.type) params = params.set('type', activityParams.type);
    // if (activityParams?.participants)
    //   params = params.set('participants', activityParams.participants);
    // this.httpClient.get<IActivity>(`${environment.boredApiUrl}`, { params });
    return this.http.getWithUrl<Attivita>(url);
  }
